# INTEIA - Dados da Empresa

## Dados Cadastrais

| Campo | Informação |
|-------|------------|
| **Razão Social** | INTEIA - INSTITUTO DE TREINAMENTO E ESTUDOS EM INTELIGENCIA ARTIFICIAL LTDA |
| **CNPJ** | 63.918.490/0001-20 |
| **Data de Inscrição** | 03/12/2025 |
| **Domínio** | inteia.com.br |

## Endereço

| Campo | Informação |
|-------|------------|
| **Logradouro** | Q SHN QUADRA 2 BLOCO F |
| **Número** | 87 |
| **Complemento** | SALA 625 E 626 |
| **CEP** | 70.702-060 |
| **Bairro** | ASA NORTE |
| **Cidade** | BRASÍLIA |
| **UF** | DF |

## Proprietário / Responsável

| Campo | Informação |
|-------|------------|
| **Nome** | Igor Morais Vasconcelos |
| **CPF** | 000.283.911-36 |
| **Telefone** | (61) 98115-7120 |
| **Email Pessoal** | igormorais123@gmail.com |
| **Email Empresarial** | igor@inteia.com.br |
| **LinkedIn** | [igor-vasconcelos-115a5032](https://www.linkedin.com/in/igor-vasconcelos-115a5032/) |
| **OAB** | 35.376 |

## Formação do Proprietário

- Doutorando em Administração Pública (IDP) - previsão 06/2026
- Mestrado em Administração Pública (IDP) - 2021
- Bacharel em Direito (UDF) - 2011
- Pós em Psicologia Positiva (IBC) - 2014
- Certificado em Prompt Engineering (IBM EdX) - 2024
- Curso de IA (USP) - 2024
- Master Coach Trainer (Metaforum International) - 2014

## Ocupação Atual

- Assessor Especial da Secretaria de Educação do DF (CPE-5) - desde 2023
- Empregado da TERRACAP (Matrícula 2268-3)
- Fundador e Diretor da INTEIA

---

## Infraestrutura Digital

### Domínio
- **Registrado em:** Registro.br
- **Domínio:** inteia.com.br
- **Status:** ✅ Configurado e funcionando

### DNS Configurado (26/01/2026)
| Tipo | Nome | Valor |
|------|------|-------|
| A | inteia.com.br | 76.76.21.21 |
| CNAME | www | inteia.com.br |
| CNAME | api | pesquisa-eleitoral-df-1.onrender.com |
| CNAME | app | cname.vercel-dns.com |
| MX | @ | aspmx.l.google.com (+ 4 alternativas) |
| TXT | @ | google-site-verification=... |
| TXT | @ | v=spf1 include:_spf.google.com ~all |
| TXT | google._domainkey | v=DKIM1; k=rsa; p=MIIBIjANBgkqhki... |

### Hospedagem
- **Frontend:** Vercel (conta paga - igormorais123)
  - URL: https://inteia.com.br
  - URL: https://www.inteia.com.br
- **Backend:** Render
  - URL: https://api.inteia.com.br (custom domain)
  - URL: https://pesquisa-eleitoral-df-1.onrender.com (direto)
- **Repositório:** GitHub - igormorais123/pesquisa-eleitoral-df

### Email Empresarial
- **Provedor:** Google Workspace
- **Email principal:** igor@inteia.com.br
- **Status:** ✅ Ativo e funcionando (domínio verificado, DKIM configurado)

---

## Senhas e Acessos

> ⚠️ **ATENÇÃO**: Preencha as senhas e tokens abaixo conforme for configurando cada serviço.
> Mantenha este arquivo SEGURO e NÃO compartilhe publicamente.

### Registro.br
- **Login:** (seu login)
- **Senha:** (sua senha)

### Google Workspace
- **Console:** https://admin.google.com
- **Conta:** igor@inteia.com.br
- **Senha:** (sua senha - guarde em local seguro)

### Vercel
- **URL:** https://vercel.com
- **Login:** igormorais123
- **Token:** (ver arquivo .env do projeto)

### Render
- **URL:** https://render.com
- **Login:** (seu login)
- **Senha:** (sua senha)

### GitHub
- **URL:** https://github.com
- **Login:** (seu login)

---

## Projetos da INTEIA

### Pesquisa Eleitoral DF 2026
- **Repositório:** pesquisa-eleitoral-df
- **Frontend:** https://inteia.com.br
- **Backend API:** https://api.inteia.com.br

---

*Última atualização: 26/01/2026*
